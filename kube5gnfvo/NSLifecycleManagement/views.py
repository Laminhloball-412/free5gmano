...
class NSLifecycleManagementViewSet(viewsets.ModelViewSet):
    ...
    def heal_ns(self, request, **kwargs):
        ...
        vnf_instance_list = list()
        heal_vnf_data = request.data.get('healVnfData', [])

        if heal_vnf_data:
            # Heal specific VNFs
            for vnf_data in heal_vnf_data:
                vnf_instance = ns_instance.NsInstance_VnfInstance.get(id=vnf_data['vnfInstanceId'])
                self._heal_vnf(vnf_instance)
                vnf_instance_list.append(vnf_instance)
        else:
            # Heal all VNFs in the NS
            for vnf_instance in ns_instance.NsInstance_VnfInstance.all():
                self._heal_vnf(vnf_instance)
                vnf_instance_list.append(vnf_instance)

        set_ns_lcm_op_occ(ns_instance, request, vnf_instance_list, self.monitor_vnf.heal)
        self.monitor_vnf.monitoring_vnf(kwargs['pk'], self.monitor_vnf.heal,
                                        vnf_instances=vnf_instance_list,
                                        container_phase='Running',
                                        ns_state=instantiated,
                                        usage_state=in_use)
    ...
    @staticmethod
    def _heal_vnf(vnf_instance):
        """Delete and re-create a VNF to restore it to healthy state."""
